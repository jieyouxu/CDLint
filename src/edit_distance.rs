...
/// letters mismatch.
pub fn find_best_match_for_name_with_substrings<'a, 'b: 'c, 'c>(
    candidates: &'a [&'b str],
    lookup: &'c str,
    dist: Option<usize>,
) -> Option<&'c str> {
...
/// We use case insensitive comparison to improve accuracy on an edge case with a lower(upper)case
/// letters mismatch.
pub fn find_best_match_for_name<'a, 'b: 'c, 'c>(
    candidates: &'a [&'b str],
    lookup: &'c str,
    dist: Option<usize>,
) -> Option<&'c str> {
...
pub fn find_best_match_for_names<'a, 'b: 'd, 'c, 'd>(
    candidates: &'a [&'b str],
    lookups: &'c [&'d str],
    dist: Option<usize>,
) -> Option<&'d str> {
    lookups
        ...
        .min_by(|(s1, r1), (s2, r2)| {
            ...
            let d2 = edit_distance(s2, r2, usize::MAX).unwrap();
            d1.cmp(&d2)
        })
        .map(|(_, r)| r)
}
...
#[cold]
fn find_best_match_for_name_impl<'a, 'b: 'c, 'c>(
    use_substring_score: bool,
    candidates: &'a [&'b str],
    lookup_symbol: &'c str,
    dist: Option<usize>,
) -> Option<&'c str> {
    let lookup = lookup_symbol;
